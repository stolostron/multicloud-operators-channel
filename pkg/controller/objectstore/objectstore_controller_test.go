...
func TestObjstoreControllerWithFakeObjectStore(t *testing.T) {
	...
	chDesc.SetObjectStorageForChannel(chn, myStoreage)
	...
	defer c.Delete(ctx, chn)
	g.Expect(c.Create(ctx, chn)).NotTo(gomega.HaveOccurred())
	...
	if _, err := myStoreage.Get(bucket, dpl.Name); err != nil {
		t.Errorf("deployable %v is not put to bucket, %#v, err: %v", dpl.Name, myStoreage, err)
	}
}
